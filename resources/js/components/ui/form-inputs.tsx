        ...
        <FormField
            ...
            render={({ field }) => (
                <FormItem>
                    ...
                    <FormControl>
                        <Textarea {...field} placeholder={placeholder} value={field.value || ''} disabled={disabled} rows={rows} />
                    </FormControl>
                    {description && <p className="text-muted-foreground text-sm">{description}</p>}
                    <FormMessage />
                </FormItem>
...
export function CheckboxInput<TFieldValues extends FieldValues>({ name, label, description, disabled }: CheckboxInputProps<TFieldValues>) {
    const { control } = useFormContext<TFieldValues>();
    ...
    return (
        <FormField
            ...
            name={name}
            render={({ field }) => (
                <FormItem className="flex flex-row items-start space-y-0 space-x-3 rounded-md p-2">
                    <FormControl>
                ...
                <FormItem>
                    ...
                    <FormControl>
                        <div className="flex items-center gap-2">
                            <Input type="color" {...field} disabled={disabled} className="h-10 w-20 p-1" />
                            {showHexInput && (
                                <Input
                                    type="text"
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-2">
                                <Label
                                    ...
                                    className="bg-primary text-primary-foreground ring-offset-background hover:bg-primary/90 focus-visible:ring-ring inline-flex h-9 cursor-pointer items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50"
                                >
                                    Browse...
                                </Label>
                                <div className="w-full rounded-md border px-3 py-2 text-sm">{fileName || 'No file selected'}</div>
                            </div>
                        </div>
                    </FormControl>
                    {description && <p className="text-muted-foreground text-sm">{description}</p>}
