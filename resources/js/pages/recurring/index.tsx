...
import { Check, ChevronDown, ChevronRight, Link2Off, Plus, Repeat, Sparkles, X } from 'lucide-react';
...
export default function RecurringIndex() {
    ...
    const fetchGroups = useCallback(async () => {
        setLoading(true);
        try {
            const { data } = await axios.get<{ data: { suggested: RecurringGroup[]; confirmed: RecurringGroup[] } }>('/api/recurring');
    ...
    const openAddTransaction = useCallback(async (group: RecurringGroup) => {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 overflow-hidden p-4 md:p-6">
                ...
                {analyticsLoading ? (
                    ...
                    analytics && (
                        <div className="bg-card flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                            <div className="flex items-center gap-3">
                                <Repeat className="text-muted-foreground h-5 w-5 shrink-0" aria-hidden />
                                <div className="min-w-0">
                                    <p className="text-muted-foreground text-sm font-medium">
                                        This month ({analytics.period.from} – {analytics.period.to})
                                    </p>
                                    <p className="text-xl font-semibold tabular-nums">{formatAmount(analytics.total_recurring, 'EUR')}</p>
                                </div>
                            </div>
                            {analytics.by_group.length > 0 && (
                                <ul className="text-muted-foreground flex min-w-0 flex-wrap gap-x-4 gap-y-1 text-sm sm:border-l sm:pl-4">
                    ...
                    <div className="flex flex-col gap-6">
                            ...
                            <CardContent className="p-6 pt-0">
                                {suggested.length === 0 ? (
                                    <div className="bg-muted/30 rounded-lg border border-dashed px-4 py-6 text-center">
                                        ...
                                        <Button className="mt-3" variant="default" onClick={handleRunDetection} disabled={detecting}>
                                            ...
                                            <li key={group.id} className="bg-background hover:bg-muted/30 rounded-lg border transition-colors">
                                                <div className="flex flex-wrap items-center justify-between gap-2 p-3">
                                                    <button
                                                        ...
                                                        className="hover:bg-muted/50 flex min-w-0 flex-1 items-center gap-2 rounded p-1 text-left"
                        ...
                        <Card className="rounded-xl border shadow-sm">
                            <CardHeader className="pb-3">
                                ...
                                <CardDescription>Recurring groups you’ve confirmed; unlink to remove the link.</CardDescription>
                            </CardHeader>
                            <CardContent className="p-6 pt-0">
                                {confirmed.length === 0 ? (
                                    <div className="bg-muted/30 rounded-lg border border-dashed px-4 py-6 text-center">
                                        <p className="text-muted-foreground text-sm">No confirmed recurring groups yet.</p>
                                        ...
                                        {confirmed.map((group) => {
                                            ...
                                            return (
                                                <li key={group.id} className="bg-background hover:bg-muted/30 rounded-lg border transition-colors">
                                                    ...
                                                    {group.stats && (
                                                        <p className="text-muted-foreground border-t px-3 pt-1 pb-2 pl-9 text-xs">
                                                    ...
                                                    {expandedId === group.id && (
                                                        <div className="border-t px-4 py-2 pt-2 pb-3 pl-9">
                                                            {group.stats && (
                                                                <div className="bg-muted/20 mb-4 rounded-lg border p-3">
            ...
            <Dialog open={addTxGroupId !== null} onOpenChange={(open) => !open && closeAddTransaction()}>
                <DialogContent className="flex max-h-[85vh] max-w-lg flex-col">
                    <DialogHeader>
                        ...
                        <DialogDescription>Select one or more unlinked transactions to add to this recurring group.</DialogDescription>
                    </DialogHeader>
                    <div className="min-h-0 flex-1 overflow-y-auto">
                        {unlinkedLoading ? (
                            ...
                            <p className="text-muted-foreground py-4 text-sm">
                                No unlinked transactions found. All transactions may already be in a subscription, or filter by the same account.
                            </p>
                        ) : (
                            <ul className="space-y-1">
                                {unlinkedTransactions.slice(0, 50).map((tx) => (
                                    <li key={tx.id}>
                                        <label className="hover:bg-muted/50 flex cursor-pointer items-center justify-between gap-2 rounded-md border p-2">
                                            <input
                                                type="checkbox"
                    ...
                    <DialogFooter>
                        ...
                        <Button type="button" onClick={handleAttachTransactions} disabled={selectedTxIds.size === 0 || attaching}>
