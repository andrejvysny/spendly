                        ...
                        {headers.map((header, colIndex) => {
                            ...
                            return (
                                <th
                                    key={colIndex}
                                    className={cn(
                                        'hover:bg-muted cursor-pointer px-3 py-2 text-left font-medium transition-colors',
                                ...
                                >
                                    ...
                                    {mappedField && <span className="text-muted-foreground mt-0.5 block text-xs">{mappedField}</span>}
                                </th>
                            );
                        })}
                    </tr>
                    ...
                    {rows.slice(0, 10).map((row, rowIndex) => (
                        <tr key={rowIndex} className="border-b last:border-0">
                            {row.map((cell, colIndex) => (
                                <td
                                    key={colIndex}
                                    className={cn('px-3 py-1.5', validationIndicators[`${rowIndex}-${colIndex}`] === false && 'bg-destructive/10')}
                                >
