...
export default function Index({
    ...
    // Component for empty data feedback
    const EmptyDataFeedback = () => {
        if (hasData || noAccountsSelected) return null;
        ...
        return (
            <div className="mb-6 rounded-lg border border-yellow-200 bg-yellow-50 p-4">
                <div className="flex items-start">
                    ...
                    <div className="ml-3">
                        <h3 className="text-sm font-medium text-yellow-800">No data found for selected period</h3>
                        {availableDataRange && (
                            <div className="mt-2 text-sm text-yellow-700">
                                <p>
                                    Your transaction data is available from{' '}
                                    <strong>{format(new Date(availableDataRange.start), 'MMM d, yyyy')}</strong> to{' '}
                                    <strong>{format(new Date(availableDataRange.end), 'MMM d, yyyy')}</strong>.
                                </p>
                                <p className="mt-1">Try selecting a different period or use the custom date range to view your data.</p>
                            </div>
                        )}
                    </div>
                </div>
